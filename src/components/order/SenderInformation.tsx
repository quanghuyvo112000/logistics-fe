...
const SenderInformation: React.FC<SenderInformationProps> = ({
  ...
  }, [senderLocation.province, senderLocation.district, setFormData, setSourceWarehouses])
    ...
    <>
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={{ xs: 12 }}>
          ...
          <LocationSelector value={senderLocation} onChange={setSenderLocation} />
          {errors.senderAddress && <FormHelperText error>{errors.senderAddress}</FormHelperText>}
        </Grid>
        <Grid size={{ xs: 12 }}>
          <Typography variant="subtitle2" gutterBottom>
            Kho gửi hàng
          ...
          {sourceWarehouses.length > 0 ? (
            <Box sx={{ p: 2, border: "1px solid #e0e0e0", borderRadius: 1, bgcolor: "#f9fafb" }}>
              <Typography variant="body1">
                {sourceWarehouses.find((w) => w.id === formData.sourceWarehouseId)?.warehouseName ||
          ...
          {errors.sourceWarehouseId && <FormHelperText error>{errors.sourceWarehouseId}</FormHelperText>}
        </Grid>
      </Grid>
    </>
  )
